...
INCREMENT=10000
for bug_id in $(seq 0 $INCREMENT 950000); do
   echo "Processing batch ${bug_id}"
   time ./kitchen.sh -file $KETTLE_JOB -level Rowlevel -param:BUG_IDS_PARTITION="(bug_id >= ${bug_id} and bug_id < (${bug_id} + $INCREMENT))" > /var/log/etl/bugzilla_etl_import_${bug_id}.log
done
