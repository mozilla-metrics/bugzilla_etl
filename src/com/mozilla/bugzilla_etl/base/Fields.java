...
public class Fields {
    ...
    public static enum Bug implements Field {
        ID("bug_id"),
        /** Only used for input, for SOLR use Version.MODIFICATION_DATE */
        REPORTED_BY,
        CREATION_DATE;
     ...
     *
     * Where columnName is null, a field are not part of the input from bugzilla, but only of the
     * output (a computed facet).
     */
    ...
    public static enum Measurement implements Field {
        /**
         * How long has the bug been OPENED before being CLOSED or OPENED before being (re-)OPENed.
         ...
         *
         * :TODO:
         * In some scenarios it might be desired to get day counts for the latest bug state (e.g.
         * to answer the question "how old are our bugs"). AFAIK the only way this can be done is
         * using date faceting on the modification_date, which will give counts for specific
         * intervals. That is usually much more useful than calculating an average.
         */
        ...
        /** A version number from 1 to n (the latest version). Useful to select the first version.*/
        NUMBER;
