...
public class RebuilderAttachmentSource {
//extends AbstractSource<Attachment> {
//
//    private boolean DEBUG_REORDER = false;
//
//    private Lookup<Attachment, KettleStepException> bugLookup;
//    public RebuilderAttachmentSource(TransformClassBase step,
//                                     RowSet bugs,
//                                     RowSet majorStatusLookup,
//                                     final Lookup<Attachment, ? extends Exception> attachmentLookup)
//    throws KettleStepException {
//        super(step, bugs);
//        // Wrap lookup to convert exception
//        this.bugLookup = new Lookup<Attachment, KettleStepException>() {
//            @Override
//            public Attachment find(Long id) throws KettleStepException {
//                try { return attachmentLookup.find(id); }
//                catch (Exception e) {
//                    String message = String.format("Trouble looking up attachment %s: %s \n",
...
//        return attachmentFromRows();
//    }
//
//    /**
//     * See also com.mozilla.bugzilla_etl.di.bug.RebuilderBugSource#bugFromRows
//     */
//    private Attachment attachmentFromRows() throws KettleValueException, KettleStepException {
//
//        // Get author and date of creation for least recent version (non-incremental).
//        final Long thebugId = input.cell(AttachmentFields.Attachment.BUG_ID).longValue();
//        final Date creationStateDate = input.cell(AttachmentFields.Attachment.SUBMISSION_DATE).dateValue();
//        final String creationStateAuthor = input.cell(AttachmentFields.Attachment.SUBMITTED_BY).stringValue();
//        final Long bugId = input.cell(AttachmentFields.Attachment.ID).longValue();
//
//        if (bugId == null) {
...
//
//        final Attachment bug = new Attachment(bugId, thebugId, creationStateAuthor, creationStateDate);
//
...
//        Pair<EnumMap<AttachmentFields.Facet, String>, Map<String, Flag>> state = stateFromBug(input.row());
//        final EnumMap<AttachmentFields.Facet, String> facetState = state.first();
//        final Map<String, Request> flagState = state.second();
//
//        if (input.cell(AttachmentFields.Activity.ATTACHMENT_ID).longValue() == null) {
...
//            bug.prepend(AttachmentVersion.latest(bug, facetState, creationStateAuthor, creationStateDate,
...
//                BugVersion successor = bug.iterator().next();
...
//        }
//
//        bug.updateFacetsAndMeasurements(null, now);
//        counter.count(bug);
//        return bug;
//    }
//
//    /**
//     * @const
//     * @return the current state facet state and review state of the attachment.
//     */
//    private Pair<EnumMap<AttachmentFields.Facet, String>, Map<String, Flag>> stateFromBug(Input.Row row)
//    throws KettleValueException {
//        // Use the "left leg" (latest bug state from bugzilla) to construct the "current" version.
//        final EnumMap<AttachmentFields.Facet, String> facetState = BugVersion.createFacets();
...
//            if (facet == AttachmentFields.Facet.REQUESTS && value != null) {
//                for (Request request : Converters.REVIEWS.parse(value)) {
//                    // flagState.put(flag.name(), flag);
//                }
//            }
//        }
//        return new Pair<EnumMap<AttachmentFields.Facet, String>, Map<String, Flag>>(facetState, flagState);
//    }
//
//    /**
...
//                                       final EnumMap<AttachmentFields.Facet, String> facetState,
...
//        do {
//            final Date currentDate = input.cell(AttachmentFields.Activity.MODIFICATION_DATE).dateValue();
...
//        } while (input.next() && bug.id().equals(input.cell(AttachmentFields.Activity.BUG_ID).longValue()));
//
...
//                                   final EnumMap<BugFields.Facet, String> facetState,
//                                   final Map<String, Flag> flagState)
//    throws KettleValueException
//    {
...
//            = candidates.get(0).cell(BugFields.Activity.MODIFICATION_DATE).dateValue();
...
//            final String author = activity.cell(BugFields.Activity.MODIFIED_BY).stringValue();
//            final BugVersion v;
...
//                v = BugVersion.latest(bug, facetState, author, date, annotation(details));
//            }
//            else {
...
//
//            final EnumMap<BugFields.Facet, String> restFacetState = facetState.clone();
...
//                String valueB = null;
//                try {
//                    valueA = rowA.cell(BugFields.Activity.MODIFIED_BY).stringValue();
//                    valueB = rowB.cell(BugFields.Activity.MODIFIED_BY).stringValue();
...
//            if (facet.isComputed) continue;
//            final String toValue = activity.cell(facet, BugFields.Facet.Column.TO).stringValue();
//            final String fromValue = activity.cell(facet, BugFields.Facet.Column.FROM).stringValue();
//            if (fromValue.isEmpty() && toValue.isEmpty()) continue;
//            if (facet == BugFields.Facet.FLAGS) {
...
//                                          activity.cell(BugFields.Activity.BUG_ID).longValue(), toFlag);
//                    }
//                    else {
...
//                                          activity.cell(BugFields.Activity.BUG_ID).longValue(), fromFlag);
//                        isConsistent = false;
//                    }
...
//                    activity.cell(BugFields.Activity.BUG_ID).longValue(), facet.name().toLowerCase(),
...
//                                              EnumMap<BugFields.Facet, String> facetState)
//    throws KettleValueException {
...
//                              i, row.cell(BugFields.Activity.MODIFIED_BY).stringValue());
//            for (BugFields.Facet facet : BugFields.Facet.values()) {
//                if (facet.isComputed) continue;
//                String from = row.cell(facet, BugFields.Facet.Column.FROM).stringValue();
//                String to = row.cell(facet, BugFields.Facet.Column.TO).stringValue();
